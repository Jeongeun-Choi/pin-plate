...
계획서 작성 후 아래 명령으로 `/tmp/tiki-taka-plan.txt`에 저장한다:

```bash
cat > /tmp/tiki-taka-plan.txt << 'PLAN_EOF'
[계획서 내용]
PLAN_EOF
```
...
매 라운드 시작 시 `/tmp/tiki-taka-plan.txt`의 최신 내용을 확인한다.

### Round 1

**Step 1 — Gemini 리뷰 요청:**

아래 내용으로 `/tmp/tiki-taka-prompt-1.txt`를 작성한다:
...
그런 다음 실행한다:
```bash
cat /tmp/tiki-taka-prompt-1.txt | gemini > /tmp/tiki-taka-gemini-1.txt
```

**Step 2 — Gemini 의견 확인 및 Claude 응답:**

`/tmp/tiki-taka-gemini-1.txt`를 읽고 Gemini의 의견을 분석한다.

- 타당한 지적 → 계획서를 수정하고 `/tmp/tiki-taka-plan.txt` 업데이트 + 동의 의견 작성
...
아래 내용으로 `/tmp/tiki-taka-prompt-2.txt`를 작성한다:
...
[최신 /tmp/tiki-taka-plan.txt 내용]

Previous rounds:
---
Round 1 - Gemini:
[/tmp/tiki-taka-gemini-1.txt 내용]
...
```bash
cat /tmp/tiki-taka-prompt-2.txt | gemini > /tmp/tiki-taka-gemini-2.txt
```

Round 1과 동일하게 Gemini 의견 확인 → Claude 응답 → 판정 확인.  
