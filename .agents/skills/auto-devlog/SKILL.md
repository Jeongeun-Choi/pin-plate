...
Codex로 템플릿 형식에 맞게 md 파일을 작성했다면 옵시디언에 저장해준다.
이때 저장 폴더 구조에 맞게 저장한다.

- 처음 저장하는 파일
  ```bash
  obsidian create name={YYYY-MM-DD}.md path="pin-plate/Daily Logs/{브랜치명}" content={Codex로 템플릿 형식에 맞게 md 파일}
  ```
- 기존에 존재하던 파일

  ```bash
  obsidian append file={YYYY-MM-DD}.md path="pin-plate/Daily Logs/{브랜치명}" content={Codex로 템플릿 형식에 맞게 md 파일}
  ```
