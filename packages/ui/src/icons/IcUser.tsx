import { SVGProps } from 'react';

export const IcUser = (props: SVGProps<SVGSVGElement>) => {
  return (
    <svg
      width="16"
      height="16"
      viewBox="0 0 16 16"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <path
        d="M12.6667 13.5733V12.24C12.6667 11.5328 12.3857 10.8545 11.8856 10.3544C11.3855 9.85429 10.7073 9.57333 10 9.57333H6.00001C5.29277 9.57333 4.61449 9.85429 4.11439 10.3544C3.61429 10.8545 3.33334 11.5328 3.33334 12.24V13.5733"
        stroke="currentColor"
        strokeWidth="1.6"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M7.99999 7.33333C9.47275 7.33333 10.6667 6.13943 10.6667 4.66667C10.6667 3.19391 9.47275 2 7.99999 2C6.52724 2 5.33333 3.19391 5.33333 4.66667C5.33333 6.13943 6.52724 7.33333 7.99999 7.33333Z"
        stroke="currentColor"
        strokeWidth="1.6"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
};
